...
export default function RoutesResults({ fromId, toId, date, time }: RoutesResultsProps) {
  ...
  const formatDate = (s: string) =>
    DateTime.fromISO(s).setLocale('sr-Latn').toFormat('d. LLL yyyy');
    ...
    <div className="bg-white rounded-lg shadow-sm border p-4">
      <div className="flex flex-wrap items-end gap-3">
        <div className="flex items-end gap-2">
          <div>
            <label className="text-xs font-medium text-gray-500 block mb-1">Od</label>
            <select
              value={editFromId}
          ...
          <div>
            <label className="text-xs font-medium text-gray-500 block mb-1">Do</label>
            <select
              value={editToId}
          ...
          <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
            <PopoverTrigger asChild>
              <Button variant="outline" className="justify-between font-normal text-sm h-9 px-2">
                {format(editDate, 'd. MMM yyyy.', { locale: srLatn })}
        ...
        <div>
          <label className="text-xs font-medium text-gray-500 block mb-1">Vreme</label>
          <input
            type="time"
            value={editTime}
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="text-lg mb-1">Nema dostupnih polazaka</p>
          <p className="text-sm">
            {getCityName(activeFromId)} → {getCityName(activeToId)},{' '}
            {formatDate(activeDate)}
            {activeTime ? `, od ${activeTime}` : ''}
          </p>
          <p className="text-sm mt-2 text-gray-400">Pokušajte sa drugim datumom ili vremenom.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <h2 className="text-lg font-semibold">
        ...
        <span className="text-gray-400 font-normal text-sm ml-2">({trips.length} polazaka)</span>
      </h2>
            ...
            <CardHeader className="pb-3">
              <div className="flex justify-between items-start">
                ...
                <Badge variant={trip.availableSeats > 0 ? 'default' : 'destructive'} className="ml-2">
                  {trip.availableSeats > 0 ? `${trip.availableSeats} slobodno` : 'Popunjeno'}
                </Badge>
              </div>
            </CardHeader>
            ...
            <CardContent>
              ...
              <div className="flex justify-between items-center">
                <div className="text-sm text-gray-600">
                  Ukupno sedišta: {trip.seatsTotal} | Slobodno: {trip.availableSeats} | Zauzeto: {trip.takenSeats.length}
                </div>
